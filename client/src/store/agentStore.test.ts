...
describe('agentStore', () => {
  ...
  beforeEach(() => {
    ...
    useAgentStore.setState({ ...getInitialState(), agents: [...mockInitialAgents], activeAgent: null, error: null, isLoading: false });
  ...
  it('should initialize with mockInitialAgents', () => {
    ...
    expect(agents).toEqual(expect.arrayContaining(mockInitialAgents.map(agent => expect.objectContaining(agent))));
  });

  it('loadAgents should replace all existing agents', () => {
    const { loadAgents } = useAgentStore.getState();
    const newAgents: AnyAgentConfig[] = [
      { id: 'new1', name: 'New Agent 1', type: AgentType.LLM, model: 'm1', instruction: 'i1', version: '1' },
    ];
  ...
  it('addAgent should add an agent to the list', () => {
    ...
    const newAgentData: Partial<AnyAgentConfig> = { name: 'Brand New Agent', type: AgentType.LLM, model: 'm2', instruction: 'i2', version: '1' };
    ...
    const agentWithId: AnyAgentConfig = { id: 'predefined-id', name: 'Agent With ID', type: AgentType.LLM, model: 'm3', instruction: 'i3', version: '1' };
    ...
    expect(agentsAfterWithId.find(a => a.id === 'predefined-id')).toEqual(agentWithId);
  ...
  it('removeAgent should clear activeAgent if it is the one being removed', () => {
    ...
    expect(useAgentStore.getState().activeAgent).toEqual(agentToActivateAndRemove);
  ...
  it('removeAgent should not affect activeAgent if a different agent is removed', () => {
    ...
    if (mockInitialAgents.length < 2) {
        console.warn("Skipping test: removeAgent needs at least 2 initial agents for this specific scenario.");
  ...
  it('updateAgent should update activeAgent if it is the one being modified', () => {
    ...
    const agentToActivateAndUpdate = JSON.parse(JSON.stringify(mockInitialAgents[0]));

    setActiveAgent(agentToActivateAndUpdate);
    expect(useAgentStore.getState().activeAgent?.id).toBe(agentToActivateAndUpdate.id);
