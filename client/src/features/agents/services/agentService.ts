 ...
 */
export interface CreateAgentAPIPayload extends Omit<LlmAgentConfig, 'id' | 'created_at' | 'updated_at' | 'user_id' | 'tools' | 'generateContentConfig'> {
  tools: ToolDTO[];
...
export const createAgent = async (
  ...
  const { generateContentConfig, knowledgeBaseIds, ...rest } = agentData;

  const apiPayload: CreateAgentAPIPayload = {
    ...rest,
    temperature: generateContentConfig?.temperature,
    max_output_tokens: generateContentConfig?.maxOutputTokens,
    top_p: generateContentConfig?.topP,
    top_k: generateContentConfig?.topK,
