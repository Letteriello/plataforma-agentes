...
vi.mock('@/components/agents/forms/LLMAgentForm', () => ({
  LLMAgentForm: () => <div data-testid="mock-llm-agent-form">LLMAgentForm Mock</div>,
}));
...
vi.mock('@/components/agents/AgentDeployTab', () => ({
  ...
  default: () => <div data-testid="mock-agent-deploy-tab">AgentDeployTab Mock</div>,
}));
    ...
    await act(async () => {
      const rendered = render(
        <MemoryRouter>
          ...
          <Toaster /> {/* Include Toaster if toast messages can be triggered on render */}
        </MemoryRouter>
      );
