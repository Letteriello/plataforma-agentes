...
import { FormProvider, useForm, UseFormReturn as RHFUseFormReturn } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { LLMAgentForm } from './LLMAgentForm';
import { agentSchema, AgentFormValues, getDefaultValues } from '@/lib/form-utils';
...
const llmDefaultValues = getDefaultValues(AgentType.LLM) as AgentFormValues & LLMAgent;
...
const TestWrapper: React.FC<{
  ...
  const contextValue: UseAgentFormReturn = {
    ...
    registerNestedField: vi.fn((prefix, name) => methods.register(`${prefix}.${name}` as any)) as any,
    handleArrayField: vi.fn(),
    setFieldValue: methods.setValue, // Use RHF's setValue directly
    getFieldValue: methods.getValues,
    getFieldError: (name) => methods.formState.errors[name as keyof AgentFormValues]?.message as string,
    hasError: (name) => !!methods.formState.errors[name as keyof AgentFormValues],
...
describe('LLMAgentForm Integration Tests', () => {
  ...
  const renderLLMAgentForm = (initialValues?: Partial<LLMAgent>, useAgentFormMockOverrides?: Partial<UseAgentFormReturn>) => {
    const renderOutput = render(
      <TestWrapper initialValues={{...llmDefaultValues, ...initialValues}} useAgentFormMockOverrides={useAgentFormMockOverrides}>
        <LLMAgentForm />
      </TestWrapper>
    );
  ...
  it('should render with initial LLM values', () => {
    const initialData = {
      name: 'My LLM Test', model: 'gemini-1.5-pro', instruction: 'Test instruction here', temperature: 0.8,
      maxTokens: 2000, topP: 0.9, topK: 30, frequencyPenalty: 0.1, presencePenalty: 0.2, systemPrompt: "System prompt test", stopSequences: ["stop1", "stop2"]
    };
    renderLLMAgentForm(initialData);

    expect(screen.getByLabelText(/agent name/i)).toHaveValue(initialData.name);
    expect(screen.getByRole('combobox', { name: /model/i })).toHaveTextContent('Gemini 1.5 Pro');
    expect(screen.getByLabelText(/instructions/i)).toHaveValue(initialData.instruction);
    ...
    expect(screen.getByLabelText(/system prompt/i)).toHaveValue(initialData.systemPrompt);
    expect(screen.getByPlaceholderText(/enter sequences separated by commas/i)).toHaveValue("stop1, stop2");
  });
  ...
  it('should display validation error for empty instruction and then clear it', async () => {
    ...
    const instructionTextarea = screen.getByLabelText(/instructions/i);
    await act(async () => { fireEvent.blur(instructionTextarea); });
    await waitFor(() => { expect(screen.getByText('Instructions are required')).toBeInTheDocument(); });

    await user.type(instructionTextarea, 'A valid instruction.');
    expect(instructionTextarea).toHaveValue('A valid instruction.');
    await waitFor(() => { expect(screen.queryByText('Instructions are required')).not.toBeInTheDocument(); });
  });
  ...
  it('should call setFieldValue for temperature change (via internal handler)', () => {
    ...
    expect(setValueSpy).toHaveBeenCalledWith('temperature', 0.9, { shouldValidate: true });
  ...
  it('should call setFieldValue for maxTokens change (via internal handler)', () => {
    ...
    expect(setValueSpy).toHaveBeenCalledWith('maxTokens', 2000, { shouldValidate: true });
  ...
  it('should update stop sequences correctly', async () => {
    ...
    const stopSequencesInput = screen.getByPlaceholderText(/enter sequences separated by commas/i);
    ...
    expect(setValueSpy).toHaveBeenCalledWith('stopSequences', ["STOP1", "STOP2", "STOP3"], expect.anything());
  });
