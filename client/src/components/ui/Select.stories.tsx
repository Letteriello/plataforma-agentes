...
const renderSelect = (args: React.ComponentProps<typeof Select> & { placeholder?: string, items: {value: string, label: string, disabled?: boolean}[], withLabel?: string, withSeparator?: boolean }) => (
  <Select {...args} onValueChange={fn(args.onValueChange || (() => {}))}>
    ...
    <SelectContent>
      ...
      <SelectGroup>
        {args.items.slice(0, args.withSeparator ? Math.ceil(args.items.length / 2) : args.items.length).map(item => (
          <SelectItem key={item.value} value={item.value} disabled={item.disabled}>
            {item.label}
          </SelectItem>
        ))}
        {args.withSeparator && <SelectSeparator />}
        {args.withSeparator && args.items.slice(Math.ceil(args.items.length / 2)).map(item => (
          <SelectItem key={item.value} value={item.value} disabled={item.disabled}>
            {item.label}
          </SelectItem>
        ))}
...
export const DisabledItem: Story = {
  ...
  args: {
    placeholder: 'Select a timezone',
    items: timeZoneItems.map(item => item.value === 'akst' ? { ...item, disabled: true } : item),
