          ...
          <div className="grid gap-4 py-4">
            <div 
                className={`border-2 border-dashed rounded-lg p-6 text-center ${
                  isUploading ? 'cursor-not-allowed bg-muted/20' : 'cursor-pointer hover:bg-muted/50'
                } transition-colors`}
            ...
            >
              ...
              />
                <Upload className={`h-10 w-10 mx-auto mb-2 ${isUploading ? 'text-muted-foreground/50' : 'text-muted-foreground'}`} />
                <p className={`text-sm font-medium mb-1 ${isUploading ? 'text-muted-foreground/50' : ''}`}>Clique para selecionar arquivos</p>
                  ...
                  {selectedFiles.map((file, index) => (
                    ...
                    >
                      <div className="flex items-center overflow-hidden">
                        <File className={`h-4 w-4 mr-2 flex-shrink-0 ${isUploading ? 'text-muted-foreground/50' : ''}`} />
                        <span className={`text-sm truncate ${isUploading ? 'text-muted-foreground/50' : ''}`}>{file.name}</span>
                      </div>
                      <Button
                        type="button"
                        variant="ghost"
